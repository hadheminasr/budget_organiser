...
export async function ActiviteService() {
  ...
  const moisActuel= `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;//padStart:pour avoir tjr 2 chiffre 5 devient 05 sert a comparer avec lastResetMonth
  //Base 
  const tousComptes  = await Account.find().lean();
  const totalComptes = tousComptes.length || 1;//pour éviter division par 0
  ...
  //Comptes actifs 7j
  const setActifs7j = new Set(//set : en js est une collection en js  qui garde seulement les valeurs uniques
    opsMois.filter(op => new Date(op.date) >= il7Jours)
      .map(op => String(op.IdAccount))//pour chaque op on garde l'id en string
  );
  const nbComptesActifs7j = setActifs7j.size;//size : propriété d'un Set qui donne le nombre d'éléments uniques
  ...
  const nbDormants= tousComptes
        .filter(c => !setActifs30j.has(String(c._id)))
        //c:compte actuel pendant le parcourt/!setActifs30j.has(String(c._id):condition:(String(c._id)):transforme id en texte/setActifs30j.has(...):verifie si l ecompte existe dans  lalliste des comptes actifs
        .length;
  const pctDormants= Math.round((nbDormants / totalComptes) * 100);
  const tauxRetention = Math.max(0, 100 - pctDormants);

  //vélocité — moyenne ops par compte actif
  const opsByAccount = opsMois.reduce((acc, op) => {//acc:accumulateur / op:opération courante
    const id = String(op.IdAccount);
    acc[id] = (acc[id] ?? 0) + 1;
    return acc;
  }, {});
  const nbComptesAvecOps  = Object.keys(opsByAccount).length || 1;//opsByAccount est un objet clé:idcompte et valeur:nbops / Object.keys(opsByAccount) donne un tableau des clés (id comptes) / .length donne le nombre de comptes avec ops / || 1 pour éviter division par 0
  const moyenneOpsParCompte  = +(nbOperationsMois / nbComptesAvecOps).toFixed(1);

  // ── Intensité moy parmi les comptes vraiment actifs (7j)
  const opsActifs7j = [...setActifs7j].map(id => opsByAccount[id] ?? 0);//le spread operator pour transformer le set en tab pour faire le map
  //["A1", "B2", "C3"].map(id => opsByAccount[id] ?? 0) devient [3, 0, 5] si A1 a 3 ops, B2 n'en a pas et C3 en a 5
  ...
  const comptesAvecObjectif = new Set(//coté js (écurité finale côté JavaScript + accès simple avec .size)
    (await Goal.find({ isActive: true }).distinct("IdAccount"))//coté mongo
      .map(String)//.map(String):tansforme les IDs MongoDB en chaînes de caractères
  );
  ...
  const profondeurUsage = [
    { feature: "Opérations", pct: Math.round((nbComptesAvecOps / totalComptes) * 100)  },
  ...
  const opsParSemaine = [];
  for (let i = 7; i >= 0; i--) {//7 semaines en arriére
    //now.getTime : retourne le temps actuelle en millisecondes/ 24 * 60 * 60 * 1000 : 1 journ en millisecondes
    
    const debutSem = new Date(now.getTime() - (i * 7 + 6) * 24 * 60 * 60 * 1000);//+6car on veut la début de la semaine 
